...
object ShapeTransformer:
  ...
  (using seu: ShapeErrorUtil): List[Shape | ErrorModel] = 
    shapes.map{s => validateColor(s)}
  def (shapes: List[Shape]).validateDim
  (using validateDim: ShapeDimensionValidator)
  (using seu: ShapeErrorUtil): List[Shape | ErrorModel] = 
    shapes.map{s => validateDim((s,0,0))}
